...
function HandleDiceREAL() {
  ThrowDice();
  AppendBoardHistory();
  var countPawnsOnBoard = CountPawnsOnBoard();
  ...
  if (countPawnsOnBoard === 0) {
    ...
    if (diceThrow === 6) {
      console.log("🚀 ~ file: index.js:142 ~ HandleDiceREAL ~ diceThrow:", diceThrow);
...
function BaseThrowCase() {
  var index = boardPlayArray.findIndex(function (e) {
    return e.colors == playerColor;
  });
...
function PlacePawn(currIndex) {
  ...
  var removedPawn = RemoveChildFromAnElement(playableSquares[currIndex]);
  if (playerColor === "blue") {
    var blueTempEnd = [36, 37, 38, 39, 0, 1];
    if (nextIndex > colorStart[playerColor] && blueTempEnd.includes(currIndex)) {
      HandleWinLane(currIndex, nextIndex, removedPawn);
    }
  }
  if (nextIndex > colorEnd[playerColor] && currIndex < colorStart[playerColor]) {
    HandleWinLane(currIndex, nextIndex, removedPawn);
    return;
  }
...
function CountPawnsOnBoard() {
  return 4 - colorFinishedPawns[playerColor].childElementCount - colorPawnsSpawn[playerColor].childElementCount;
}
